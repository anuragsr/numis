		...
		var _NaNExp = /[^\d\-\.]/g,
			...
			_getRule = CSSRulePlugin.getRule = function(selector) {
				...
				while (--i > -1) {
					...
					j = curSS.length;
					while (--j > -1) {
						if ((typeof curSS[j].selectorText !== "undefined")&&(","+curSS[j].selectorText.split("::").join(":")+",").indexOf(selector) !== -1) { //note: IE adds an extra ":" to pseudo selectors, so .myClass:after becomes .myClass::after, so we need to strip the extra one out.
							if (pseudo) {
